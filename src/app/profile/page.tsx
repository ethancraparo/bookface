...
function Toggle({ enabled, onChange, label, hint }: {
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-1">
      ...
      >
        <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-[#ffffff] shadow-sm transition-transform duration-200 ${enabled ? 'translate-x-5' : 'translate-x-0'}`} />
      </button>
    </div>
  );
}
